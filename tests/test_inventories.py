...
def test_inventory_add_host(requests_add_host_inventory, inventory_host_request_schema):
    inventory = Inventory(base_url="https://localhost:5000", username="test", password="test")
    response = inventory.add_host(inventory_id=38, host=inventory_host_request_schema)


def test_inventory_add_host_bad(requests_add_host_inventory, inventory_host_request_schema):
    inventory = Inventory(base_url="https://localhost:5000", username="test", password="test")
    with pytest.raises(TypeError):
        inventory.add_host(inventory_id=38, host="bad")

    with pytest.raises(TypeError):
        inventory.add_host(inventory_id="bad", host=inventory_host_request_schema)


def test_inventory_add_group(requests_add_group_inventory, inventory_group_request_schema):
    inventory = Inventory(base_url="https://localhost:5000", username="test", password="test")
    response = inventory.add_group(inventory_id=38, group=inventory_group_request_schema)


def test_inventory_add_group_bad(requests_add_group_inventory, inventory_group_request_schema):
    inventory = Inventory(base_url="https://localhost:5000", username="test", password="test")
    with pytest.raises(TypeError):
        inventory.add_group(inventory_id=38, group="bad")

    with pytest.raises(TypeError):
        inventory.add_group(inventory_id="bad", group=inventory_group_request_schema)
