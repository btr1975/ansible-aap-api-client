import pytest
from ansible_aap_api_client import AAPClient


def test_get_all_groups(requests_get_two_group):
    obj = AAPClient(base_url="https://localhost:5000", username="test", password="test", ssl_verify=False)
    response = obj.get_all_groups()


def test_get_group(requests_get_two_group):
    obj = AAPClient(base_url="https://localhost:5000", username="test", password="test")
