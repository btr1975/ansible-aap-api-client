...
def test_group_add_host(requests_add_host_group, inventory_host_request_schema):
    obj = Group(base_url="https://localhost:5000", username="test", password="test")
    response = obj.add_host(group_id=2, host=inventory_host_request_schema)


def test_group_add_host_bad(requests_add_host_group, inventory_host_request_schema):
    obj = Group(base_url="https://localhost:5000", username="test", password="test")
    with pytest.raises(TypeError):
        obj.add_host(group_id=2, host="bad")

    with pytest.raises(TypeError):
        obj.add_host(group_id="bad", host=inventory_host_request_schema)
